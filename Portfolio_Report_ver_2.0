...
def get_gspread_client():
    """
    ServiceAccountCredentials + gspread 로 Google Sheets 클라이언트를 생성한다.
    필요 환경변수:
...
def get_fx_multipliers(base_currency):
    """
    BaseCurrency 에 따라 USD/CAD → 기준통화 변환 계수를 리턴.
...
def load_portfolio_from_gsheet():
    ...
    새로운 구조:
        ...
        (기존 CashUSD 만 있던 경우 fallback)

    반환:
        df_hold, tfsa_cash_usd, resp_cash_cad, base_currency
    """
    ...
    settings = dict(
        zip(
            df_settings["Key"].astype(str),
            df_settings["Value"],
        )
    )

    # 신규: TFSA_CashUSD / RESP_CashCAD / BaseCurrency
    ...
    return df_hold, tfsa_cash_usd, resp_cash_cad, base_currency
...
def enrich_holdings_with_prices(df_hold, base_currency, tfsa_cash_usd, resp_cash_cad):
    """
    ...
    반환:
        ...
        - df_enriched: 각 종목별 상세 정보 (기준통화 기준)
        - account_summary: 계좌별 요약 dict
    """
    df = df_hold.copy()
    ...
    summary["TOTAL"] = {
        ...
        "cash_native": summary["TFSA"]["cash_native"] + summary["RESP"]["cash_native"],
...
def build_html_report(df_enriched, account_summary):
    ...
    df_view["ProfitLossPct"] = df_view["ProfitLossPct"].map(fmt_pct)
...
def main():
    ...
    df_hold, tfsa_cash_usd, resp_cash_cad, base_currency = load_portfolio_from_gsheet()
