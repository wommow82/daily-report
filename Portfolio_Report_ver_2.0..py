...
def _safe_requests_get_json(url, params=None, headers=None, timeout=25, retries=3, backoff=2.0):
...
def fetch_house_stockwatcher_trades():
    """HouseStockWatcher 공개 API(키 불필요)를 시도한다. 실패 시 None."""
    # 주의: 서비스 가용성에 따라 502/다운 가능
    url = "https://housestockwatcher.com/api"
    return _safe_requests_get_json(url)


def fetch_fmp_house_trades_by_name(name: str):
    """FMP House Trades By Name. FMP_API_KEY가 없으면 None."""
    api_key = os.environ.get("FMP_API_KEY")
    if not api_key:
        return None
    url = "https://financialmodelingprep.com/stable/house-trades-by-name"
    params = {"name": name, "apikey": api_key}
    return _safe_requests_get_json(url, params=params)
...
def get_pelosi_trades(days=120, max_rows=25):
    ...
    우선순위:
      1) HouseStockWatcher (무료/키 불필요) → 장애 시 빈값 가능
      2) FMP house-trades-by-name (키 필요; Free plan 가능성은 있으나 계정 정책에 좌우)
    반환: 정규화된 dict 리스트 (최신순)
    """
    name = os.environ.get("PELOSI_NAME", "Nancy Pelosi").strip()
    name_l = name.lower()

    # 1) HouseStockWatcher
    rows = _normalize_trade_rows(fetch_house_stockwatcher_trades())

    # 2) FMP fallback
    if not rows:
        rows = _normalize_trade_rows(fetch_fmp_house_trades_by_name(name))
    ...
    norm = []
    for r in filtered:
        filed = parse_dt(_pick(r, ["disclosure_date", "filingDate", "filedDate", "reportDate", "disclosureDate"]))
        trade = parse_dt(_pick(r, ["transaction_date", "transactionDate", "tradeDate", "transactionDateStart", "transaction_date_start"]))
        # 소스에 따라 start/end로 나뉘는 경우가 있어 start 우선
        if pd.isna(trade):
            trade = parse_dt(_pick(r, ["transaction_date_end", "transactionDateEnd", "transactionDateEnd"]))
    ...
    # 최신순 정렬 (filed 우선)
    kept.sort(key=lambda x: (x[0] if not pd.isna(x[0]) else pd.Timestamp.min,
                            x[1] if not pd.isna(x[1]) else pd.Timestamp.min), reverse=True)

    return [r for _, _, r in kept[:max_rows]]
...
def build_pelosi_trades_section_html(df_enriched, days=120, max_rows=25):
    ...
    trades = get_pelosi_trades(days=days, max_rows=max_rows)

    # 데이터가 없을 때도 리포트는 계속 진행
    if not trades:
        key_hint = ""
        if not os.environ.get("FMP_API_KEY"):
            key_hint = "<br/>- 옵션: FMP_API_KEY 설정 시 FMP 엔드포인트로 fallback 가능합니다."
        return (
            "<p class='muted'>Pelosi 트레이드 데이터를 가져오지 못했습니다."
            "<br/>- HouseStockWatcher 서비스 장애/응답 오류(예: 502) 또는 데이터 소스 제한 가능"
            f"{key_hint}</p>"
        )
    ...
    rows = []
    for t in trades:
        symbol = str(_pick(t, ["symbol", "ticker", "assetTicker", "asset_ticker"], "")).upper().strip()
        tx_type = str(_pick(t, ["type", "transaction_type", "transactionType"], "")).strip()
        amount = str(_pick(t, ["amount", "amount_range", "amountRange", "range"], "")).strip()
        owner = str(_pick(t, ["owner", "owner_type", "ownerType"], "")).strip()
        filed = _pick(t, ["disclosure_date", "filingDate", "filedDate", "reportDate", "disclosureDate"], "")
        trade_dt = _pick(t, ["transaction_date", "transactionDate", "tradeDate", "transactionDateStart"], "")

        in_pf = symbol in portfolio_tickers and symbol != ""
        sym_html = f"<b>{symbol}</b>" if in_pf else (symbol if symbol else "-")

        rows.append(
            {
                "Filed": str(filed)[:10] if filed else "-",
                "TradeDate": str(trade_dt)[:10] if trade_dt else "-",
                "Symbol": sym_html,
                "Type": tx_type or "-",
                "Amount": amount or "-",
                "Owner": owner or "-",
                "InMyPortfolio": "✅" if in_pf else "",
            }
        )
    ...
    # My holdings와 겹치는 종목만 별도 요약
    hits = df[df["InMyPortfolio"] == "✅"]
    hit_html = ""
    if not hits.empty:
        # 중복 제거: Symbol 기준 최신 1개
        hits2 = hits.copy()
        # Symbol 컬럼은 HTML(bold)이라 정렬/중복 제거용 raw_symbol 재구성
        ...
        hit_html = (
            "<p><b>내 보유 종목과 겹치는 최근 공시:</b></p>"
            + hits2.to_html(index=False, escape=False)
        )

    # 전체 최근 리스트
    table_html = df.to_html(index=False, escape=False)

    return hit_html + "<p><b>최근 Pelosi 관련 공시(최신순):</b></p>" + table_html
